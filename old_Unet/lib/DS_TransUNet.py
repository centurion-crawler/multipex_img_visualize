...
class UNet(nn.Module):
    def __init__(self, dim, n_class, in_ch=37):
        super().__init__()
        self.init_layer = nn.Conv2d(in_channels=in_ch, out_channels=3, kernel_size=5,stride=1,padding=2,bias=False)
        ...
        self.layer1_cls = Swin_Decoder(8*dim, 2, 8)
        self.layer2_cls = Swin_Decoder(4*dim, 2, 4)
        self.layer3_cls = Swin_Decoder(2*dim, 2, 2)
        self.layer4_cls = Decoder(dim, dim, dim//2)
        self.layer5_cls = Decoder(dim//2, dim//2, dim//4)
        self.final_cls = nn.Conv2d(dim//4, n_class, kernel_size=1, stride=1, padding=0)

        # self.layer1_cell = Swin_Decoder(8*dim, 2, 8)
        # self.layer2_cell = Swin_Decoder(4*dim, 2, 4)
        # self.layer3_cell = Swin_Decoder(2*dim, 2, 2)
        # self.layer4_cell = Decoder(dim, dim, dim//2)
        # self.layer5_cell = Decoder(dim//2, dim//2, dim//4)
        # self.final_cell = nn.Conv2d(dim//4, 2, kernel_size=1, stride=1, padding=0)

        self.down1 = nn.Conv2d(3, dim//4, kernel_size=1, stride=1, padding=0)
        self.down2 = conv_block(dim//4, dim//2)
        

        self.loss1_cls = nn.Sequential(
                nn.Conv2d(dim*8, n_class, kernel_size=1, stride=1, padding=0),
                nn.ReLU(),
                nn.Upsample(scale_factor=32)
        )

        # self.loss1_cell = nn.Sequential(
        #         nn.Conv2d(dim*8, 2, kernel_size=1, stride=1, padding=0),
        #         nn.ReLU(),
        #         nn.Upsample(scale_factor=32)
        # )

        self.loss2_cls = nn.Sequential(
                nn.Conv2d(dim, n_class, kernel_size=1, stride=1, padding=0),
                nn.ReLU(),
                nn.Upsample(scale_factor=4)
        )
        # self.loss2_cell = nn.Sequential(
        #         nn.Conv2d(dim, 2, kernel_size=1, stride=1, padding=0),
        #         nn.ReLU(),
        #         nn.Upsample(scale_factor=4)
        # )
    ...
    def forward(self, x_in):
        x3= self.init_layer(x_in).relu()
        out = self.encoder(x3)
        out2 = self.encoder2(x3)
        ...
        e3 = self.change3(e3)
        e4 = self.change4(e4)
        loss1_cls = self.loss1_cls(e4)
        # loss1_cell = self.loss1_cell(e4)
        
        ds1 = self.down1(x3)
        ds2 = self.down2(ds1)

        d1_cls = self.layer1_cls(e4, e3)
        d2_cls = self.layer2_cls(d1_cls, e2)
        d3_cls = self.layer3_cls(d2_cls, e1)
        loss2_cls = self.loss2_cls(d3_cls)
        d4_cls = self.layer4_cls(d3_cls, ds2)
        d5_cls = self.layer5_cls(d4_cls, ds1)
        o_cls = self.final_cls(d5_cls)

        # d1_cell = self.layer1_cell(e4, e3)
        # d2_cell = self.layer2_cell(d1_cell, e2)
        # d3_cell = self.layer3_cell(d2_cell, e1)
        # loss2_cell = self.loss2_cell(d3_cell)
        # d4_cell = self.layer4_cell(d3_cell, ds2)
        # d5_cell = self.layer5_cell(d4_cell, ds1)
        # o_cell = self.final_cell(d5_cell)

        # return o_cls, o_cell, loss1_cls, loss1_cell, loss2_cls, loss2_cell
        return o_cls, loss1_cls, loss2_cls, 
