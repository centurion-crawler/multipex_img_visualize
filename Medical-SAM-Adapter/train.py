    ...
    for epoch in range(500):
        ...
        if epoch and epoch < 5:
            if args.dataset != 'REFUGE':
                tol, eiou_edice = function.validation_sam(args, nice_test_loader, epoch, net, writer)
                print(f"Total score: {tol}")
                for i in range(1,2):
                    e_dice.append(eiou_edice[i*2+1])
                    logger.info(f'class {i}, IOU: {eiou_edice[i*2]}, DICE: {eiou_edice[i*2+1]} || @ epoch {epoch}.')
            # else:
