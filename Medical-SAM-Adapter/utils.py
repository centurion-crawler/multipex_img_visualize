...
def vis_image(imgs, pred_masks, gt_masks, save_path, reverse = False, points = None, boxes = None):
    ...
    if c == 2: # for REFUGE multi mask output
        ...
        tup = (pred_disc[:row_num,:,:,:], pred_cup[:row_num,:,:,:], gt_disc[:row_num,:,:,:], gt_cup[:row_num,:,:,:])
        # for t in tup:
            # print(t.shape)
        ...
        for i in range(0, c):
            ...
            gts.append(gt)
        tup = [imgs[:row_num,i,:,:].unsqueeze(1).expand(b,3,h,w)] + preds + gts
        # for t in tup:
        #     print(t.shape)
        compose = torch.cat(tup,0)
        vutils.save_image(compose, fp = save_path, nrow = row_num, padding = 10)
...
def eval_seg(pred,true_mask_p,threshold):
    ...
    if c == 2:
        ...
        for th in threshold:
            ...
            vpred_cpu = vpred.cpu()
            for i in range(0, c):
                pred_ = vpred_cpu[:,i,:,:].numpy().astype('int32')
                mask = gt_vmask_p[:,i,:,:].squeeze(1).cpu().numpy().astype('int32')
        
                '''iou for numpy'''
                ious[i] += iou(pred_,mask)
...
def random_click(mask, point_labels = 1,point_num=1):
    ...
    if point_num <= len(indices):
        idx = np.array(random.sample(list(range(len(indices))),k=point_num))
    return point_labels, indices[idx]
